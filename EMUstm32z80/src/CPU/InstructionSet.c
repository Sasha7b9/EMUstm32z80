#include "InstructionSet.h"
#include "registers.h"
#include "../RAM.h"
#include "DAA.h"


/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
int LD_R_R(void)
{
    R8_HI(prevPC) = R8_LO(prevPC);

    return 4;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int LD_R_N(void)
{
    R8_HI(prevPC) = PCandInc();

    return 7;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int LD_R_pHL(void)
{
    R8_HI(prevPC) = pHL;

    return 7;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int LD_pHL_R(void)
{
    pHL = R8_LO(prevPC);

    return 7;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int LD_pHL_N(void)
{
    pHL = PCandInc();

    return 10;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int LD_A_pBC(void)
{
    A = RAM[BC];

    return 7;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int LD_A_pDE(void)
{
    A = RAM[DE];

    return 7;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int LD_A_NN(void)
{
    A = RAM[PC16andInc()];

    return 13;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int LD_pBC_A(void)
{
    RAM[BC] = A;

    return 7;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int LD_pDE_A(void)
{
    RAM[DE] = A;

    return 7;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int LD_pNN_A(void)
{
    RAM[PC16andInc()] = A;

    return 13;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int LD_DD_NN(void)
{
    DD_45(prevPC) = PC16andInc();

    return 10;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int LD_pNN_HL(void)
{
    uint16 address = PC16andInc();

    RAM[address] = L;
    RAM[address + 1] = H;

    return 16;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int LD_SP_HL(void)
{
    SP = HL;

    return 6;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int PUSH_QQ(void)
{
    RAM[SP - 2] = (uint8)QQ_45(prevPC);
    RAM[SP - 1] = (uint8)(QQ_45(prevPC) >> 8);
    SP -= 2;

    return 11;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int POP_QQ(void)
{
    QQ_45(prevPC) = RAM[SP] + RAM[SP + 1] * 256;
    SP += 2;

    return 10;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int EX_DE_HL(void)
{
    uint16 temp;

    EXCH(DE, HL);

    return 4;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int EX_AF_AFalt(void)
{
    uint8 temp;

    EXCH(A, Aalt);
    EXCH(F, RFalt);

    return 4;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int EXX(void)
{
    uint8 temp;

    EXCH(B, Balt);
    EXCH(C, RCalt);
    EXCH(D, RDalt);
    EXCH(E, REalt);
    EXCH(H, RHalt);
    EXCH(L, RLalt);

    return 4;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int EX_pSP_HL(void)
{
    uint8 temp;

    EXCH(H, RAM[SP + 1]);
    EXCH(L, RAM[SP]);

    return 9;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int ADD_A_R(void)
{
    A += R8_LO(prevPC);

    return 4;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int ADD_A_N(void)
{
    A += PCandInc();

    return 7;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int ADD_A_pHL(void)
{
    A += pHL;

    return 7;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int ADC_A_S(void)
{
    A += R8_LO(prevPC) + CF;

    return 4;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int ADC_A_N(void)
{
    A += PCandInc() + CF;

    return 7;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int ADC_A_pHL(void)
{
    A += pHL + CF;

    return 7;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int SUB_S(void)
{
    A -= R8_LO(prevPC);

    return 4;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int SUB_N(void)
{
    A -= PCandInc();

    return 7;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int SUB_pHL(void)
{
    A -= pHL;

    return 7;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int LD_HL_pNN(void)
{
    HL = PC16andInc();

    return 16;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int SBC_A_S(void)
{
    A -= R8_LO(prevPC) - CF;

    return 4;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int SBC_A_N(void)
{
    A -= PCandInc() - CF;

    return 7;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int SBC_A_pHL(void)
{
    A -= pHL - CF;

    return 7;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int AND_S(void)
{
    A &= R8_LO(prevPC);

    return 4;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int AND_N(void)
{
    A &= PCandInc();

    return 7;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int AND_pHL(void)
{
    A &= pHL;

    return 7;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int OR_S(void)
{
    A |= R8_LO(prevPC);

    return 4;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int OR_N(void)
{
    A |= PCandInc();

    return 7;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int OR_pHL(void)
{
    A |= pHL;

    return 7;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int XOR_S(void)
{
    A ^= R8_LO(prevPC);

    return 4;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int XOR_N(void)
{
    A ^= PCandInc();

    return 7;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int XOR_pHL(void)
{
    A ^= pHL;

    return 7;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int CP_S(void)
{
    uint8 value = R8_LO(prevPC);

    SET_N;

    if(A == value)
    {
        SET_Z;
    }
    else
    {
        RES_Z;
    }

    if(A < value)
    {
        SET_C;
        SET_S;
    }
    else
    {
        RES_C;
        RES_S;
    }

    if((value & 0x0f) > (A & 0x0f))
    {
        SET_H;
    }
    else
    {
        RES_H;
    }

    return 4;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int CP_N(void)
{
    return 7;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int CP_pHL(void)
{
    return 7;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int INC_R(void)
{
    R8_HI(prevPC) += 1;

    // + + x + x v 0 .

    RES_N;

    /*
    */

    return 4;
}

//------------------------------------------------------------------------------------------------------------------------------------------------------
int INC_pHL(void)
{
    pHL += 1;

    // + + x + x v 0 .

    RES_N;

    /*
    */

    return 7;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int DEC_M(void)
{
    R8_HI(prevPC) -= 1;

    // + + x + x v 1 .

    SET_N;

    /*
    */

    return 4;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int DEC_pHL(void)
{
    pHL += 1;

    // + + x + x v 1 .

    SET_N;

    /**/

    return 7;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int CPL(void)
{
    A = ~A;

    // . . x + x . 1 .

    // H WARN

    SET_N;

    return 4;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int CCF(void)
{
    if(CF)
    {
        RES_C;
    }
    else
    {
        SET_C;
    }

    // . . x x x . 0 +

    RES_N;

    return 4;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int SCF(void)
{
    SET_C;

    // . . x 0 x . 0 +

    RES_H;
    RES_N;

    return 4;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int NOP(void)
{
    return 4;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int HALT(void)
{
    return 0;   // WARN There halt CPU
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int DI(void)
{
    IFF1 = 0;
    IFF2 = 0;

    return 4;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int EI(void)
{
    IFF1 = 1;
    IFF2 = 1;

    return 4;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int ADD_HL_SS(void)
{
    HL += SS_45(prevPC);

    // . . x x x . 0 +

    RES_N;

    // C WARN

    return 11;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int INC_SS(void)
{
    SS_45(prevPC) += 1;

    return 6;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int DEC_SS(void)
{
    SS_45(prevPC) -= 1;

    return 6;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int RLCA(void)
{
    uint8 hiBit = GET_BIT(A, 7);

    A <<= 1;

    LOAD_BIT(A, 0, hiBit);
    LOAD_C(hiBit);

    // . . x 0 x . 0 +
    RES_H;
    RES_N;

    return 4;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int RLA(void)
{
    uint8 hiBit = GET_BIT(A, 7);
    uint8 oldCY = CF;

    A <<= 1;
    LOAD_C(hiBit);
    LOAD_BIT(A, 0, oldCY);

    // . . x 0 x . 0 +
    RES_H;
    RES_N;

    return 4;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int RRCA(void)
{
    uint8 loBit = GET_BIT(A, 0);

    A >>= 1;
    LOAD_BIT(A, 7, loBit);
    LOAD_C(loBit);

    // . . x 0 x . 0 +
    RES_H;
    RES_N;

    return 4;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int RRA(void)
{
    uint8 loBit = GET_BIT(A, 0);
    uint8 oldCY = CF;

    A >>= 1;
    LOAD_BIT(A, 7, oldCY);
    LOAD_C(loBit);

    // . . x 0 x . 0 +
    RES_H;
    RES_N;

    return 4;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int JP_NN(void)
{
    SetPC(PC16andInc());

    return 10;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
static void FuncNZ(uint16 address)
{
    if(ZF != 1)
    {
        SetPC(address);
    }
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
static void FuncZ(uint16 address)
{
    if(ZF == 1)
    {
        SetPC(address);
    }
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
static void FuncNC(uint16 address)
{
    if(CF != 1)
    {
        SetPC(address);
    }
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
static void FuncC(uint16 address)
{
    if(CF == 1)
    {
        SetPC(address);
    }
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
static void FuncPO(uint16 address)
{
    if(PF != 1)
    {
        SetPC(address);
    }
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
static void FuncPE(uint16 address)
{
    if(PF == 1)
    {
        SetPC(address);
    }
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
static void FuncP(uint16 address)
{
    if(ZF != 1)
    {
        SetPC(address);
    }
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
static void FuncM(uint16 address)
{
    if(ZF == 1)
    {
        SetPC(address);
    }
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int JP_CC_NN(void)
{
    const pFuncVU16 funcs[8] =
    {
        FuncNZ,
        FuncZ,
        FuncNC,
        FuncC,
        FuncPO,
        FuncPE,
        FuncP,
        FuncM
    };

    uint cond = CC(prevPC);

    uint16 address = PC16andInc();

    funcs[cond](address);

    return 10;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int JR_E(void)
{
    AddPC(PCandInc());

    return 12;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int JR_C_E(void)
{
    uint8 address = PCandInc();
    if(CF)
    {
        AddPC(address);
        return 12;
    }

    return 7;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int JR_NC_E(void)
{
    uint8 address = PCandInc();
    if(CF == 0)
    {
        AddPC(address);
        return 12;
    }

    return 7;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int JR_Z_E(void)
{
    uint8 address = PCandInc();
    if(ZF)
    {
        AddPC(address);
        return 12;
    }

    return 7;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int JR_NZ_E(void)
{
    uint8 address = PCandInc();
    if(ZF == 0)
    {
        AddPC(address);
        return 12;
    }
    return 7;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int JP_pHL(void)
{
    SetPC(HL);
    return 4;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int DJNZ_E(void)
{
    B -= 1;
    
    uint8 delta = PCandInc();

    if(B)
    {
        AddPC(delta);
        return 13;
    }

    return 8;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int CALL_NN(void)
{
    uint16 valuePC = ValuePC();

    RAM[SP - 1] = (uint8)(valuePC >> 8);
    RAM[SP - 2] = (uint8)(valuePC);
    SetPC(PC16andInc());

    return 17;
}


//------------------------------------------------------------------------------------------------------------------------------------------------------
int RunCommand(void)
{
    const pFuncIV firstLevel[256] =
    {
        /*   00 000 000   */ NOP,
        /*   00 000 010   */ LD_pBC_A,
        /*   00 000 011   */ INC_SS,
        /*   00 000 001   */ LD_DD_NN,
        /*   00 000 100   */ INC_R,
        /*   00 000 101   */ DEC_M,
        /*   00 000 110   */ LD_R_N,
        /*   00 000 111   */ RLCA,
        /*   00 001 000   */ EX_AF_AFalt,
        /*   00 001 001   */ ADD_HL_SS,
        /*   00 001 010   */ LD_A_pBC,
        /*   00 001 011   */ DEC_SS,
        /*   00 001 100   */ INC_R,
        /*   00 001 101   */ DEC_M,
        /*   00 001 110   */ LD_R_N,
        /*   00 001 111   */ RRCA,
        /*   00 010 000   */ DJNZ_E,
        /*   00 010 001   */ LD_DD_NN,
        /*   00 010 010   */ LD_pDE_A,
        /*   00 010 011   */ INC_SS,
        /*   00 010 100   */ INC_R,
        /*   00 010 101   */ DEC_M,
        /*   00 010 110   */ LD_R_N,
        /*   00 010 111   */ RLA,
        /*   00 011 000   */ JR_E,
        /*   00 011 001   */ ADD_HL_SS,
        /*   00 011 010   */ LD_A_pDE,
        /*   00 011 011   */ DEC_SS,
        /*   00 011 100   */ INC_R,
        /*   00 011 101   */ DEC_M,
        /*   00 011 110   */ LD_R_N,
        /*   00 011 111   */ RRA,
        /*   00 100 000   */ JR_NZ_E,
        /*   00 100 001   */ LD_DD_NN,
        /*   00 100 010   */ LD_pNN_HL,
        /*   00 100 011   */ INC_SS,
        /*   00 100 100   */ INC_R,
        /*   00 100 101   */ DEC_M,
        /*   00 100 110   */ LD_R_N,
        /*   00 100 111   */ DAA,
        /*   00 101 000   */ JR_Z_E,
        /*   00 101 001   */ ADD_HL_SS,
        /*   00 101 010   */ LD_HL_pNN,
        /*   00 101 011   */ DEC_SS,
        /*   00 101 100   */ INC_R,
        /*   00 101 101   */ DEC_M,
        /*   00 101 110   */ LD_R_N,
        /*   00 101 111   */ CPL,
        /*   00 110 000   */ JR_NC_E,
        /*   00 110 001   */ LD_DD_NN,
        /*   00 110 010   */ LD_pNN_A,
        /*   00 110 011   */ INC_SS,
        /*   00 110 100   */ INC_pHL,
        /*   00 110 101   */ DEC_pHL,
        /*   00 110 110   */ LD_pHL_N,
        /*   00 110 111   */ SCF,
        /*   00 111 000   */ JR_C_E,
        /*   00 111 001   */ ADD_HL_SS,
        /*   00 111 010   */ LD_A_NN,
        /*   00 111 011   */ DEC_SS,
        /*   00 111 100   */ INC_R,
        /*   00 111 101   */ DEC_M,
        /*   00 111 110   */ LD_R_N,
        /*   00 111 111   */ CCF,

        /*   01 000 000   */ LD_R_R,
        /*   01 000 001   */ LD_R_R,
        /*   01 000 010   */ LD_R_R,
        /*   01 000 011   */ LD_R_R,
        /*   01 000 100   */ LD_R_R,
        /*   01 000 101   */ LD_R_R,
        /*   01 000 110   */ LD_R_pHL,
        /*   01 000 111   */ LD_R_R,
        /*   01 001 000   */ LD_R_R,
        /*   01 001 001   */ LD_R_R,
        /*   01 001 010   */ LD_R_R,
        /*   01 001 011   */ LD_R_R,
        /*   01 001 100   */ LD_R_R,
        /*   01 001 101   */ LD_R_R,
        /*   01 001 110   */ LD_R_pHL,
        /*   01 001 111   */ LD_R_R,
        /*   01 010 000   */ LD_R_R,
        /*   01 010 001   */ LD_R_R,
        /*   01 010 010   */ LD_R_R,
        /*   01 010 011   */ LD_R_R,
        /*   01 010 100   */ LD_R_R,
        /*   01 010 101   */ LD_R_R,
        /*   01 010 110   */ LD_R_pHL,
        /*   01 010 111   */ LD_R_R,
        /*   01 011 000   */ LD_R_R,
        /*   01 011 001   */ LD_R_R,
        /*   01 011 010   */ LD_R_R,
        /*   01 011 011   */ LD_R_R,
        /*   01 011 100   */ LD_R_R,
        /*   01 011 101   */ LD_R_R,
        /*   01 011 110   */ LD_R_pHL,
        /*   01 011 111   */ LD_R_R,
        /*   01 100 000   */ LD_R_R,
        /*   01 100 001   */ LD_R_R,
        /*   01 100 010   */ LD_R_R,
        /*   01 100 011   */ LD_R_R,
        /*   01 100 100   */ LD_R_R,
        /*   01 100 101   */ LD_R_R,
        /*   01 100 110   */ LD_R_pHL,
        /*   01 100 111   */ LD_R_R,
        /*   01 101 000   */ LD_R_R,
        /*   01 101 001   */ LD_R_R,
        /*   01 101 010   */ LD_R_R,
        /*   01 101 011   */ LD_R_R,
        /*   01 101 100   */ LD_R_R,
        /*   01 101 101   */ LD_R_R,
        /*   01 101 110   */ LD_R_pHL,
        /*   01 101 111   */ LD_R_R,
        /*   01 110 000   */ LD_pHL_R,
        /*   01 110 001   */ LD_pHL_R,
        /*   01 110 010   */ LD_pHL_R,
        /*   01 110 011   */ LD_pHL_R,
        /*   01 110 100   */ LD_pHL_R,
        /*   01 110 101   */ LD_pHL_R,
        /*   01 110 110   */ HALT,
        /*   01 110 111   */ LD_pHL_R,
        /*   01 111 000   */ LD_R_R,
        /*   01 111 001   */ LD_R_R,
        /*   01 111 010   */ LD_R_R,
        /*   01 111 011   */ LD_R_R,
        /*   01 111 100   */ LD_R_R,
        /*   01 111 101   */ LD_R_R,
        /*   01 111 110   */ LD_R_pHL,
        /*   01 111 111   */ LD_R_R,

        /*   10 000 000   */ ADD_A_R,
        /*   10 000 001   */ ADD_A_R,
        /*   10 000 010   */ ADD_A_R,
        /*   10 000 011   */ ADD_A_R,
        /*   10 000 100   */ ADD_A_R,
        /*   10 000 101   */ ADD_A_R,
        /*   10 000 110   */ ADD_A_pHL,
        /*   10 000 111   */ ADD_A_R,
        /*   10 001 000   */ ADC_A_S,
        /*   10 001 001   */ ADC_A_S,
        /*   10 001 010   */ ADC_A_S,
        /*   10 001 011   */ ADC_A_S,
        /*   10 001 100   */ ADC_A_S,
        /*   10 001 101   */ ADC_A_S,
        /*   10 001 110   */ ADC_A_pHL,
        /*   10 001 111   */ ADC_A_S,
        /*   10 010 000   */ SUB_S,
        /*   10 010 001   */ SUB_S,
        /*   10 010 010   */ SUB_S,
        /*   10 010 011   */ SUB_S,
        /*   10 010 100   */ SUB_S,
        /*   10 010 101   */ SUB_S,
        /*   10 010 110   */ SUB_pHL,
        /*   10 010 111   */ SUB_S,
        /*   10 011 000   */ SBC_A_S,
        /*   10 011 001   */ SBC_A_S,
        /*   10 011 010   */ SBC_A_S,
        /*   10 011 011   */ SBC_A_S,
        /*   10 011 100   */ SBC_A_S,
        /*   10 011 101   */ SBC_A_S,
        /*   10 011 110   */ SBC_A_pHL,
        /*   10 011 111   */ SBC_A_S,
        /*   10 100 000   */ AND_S,
        /*   10 100 001   */ AND_S,
        /*   10 100 010   */ AND_S,
        /*   10 100 011   */ AND_S,
        /*   10 100 100   */ AND_S,
        /*   10 100 101   */ AND_S,
        /*   10 100 110   */ AND_pHL,
        /*   10 100 111   */ AND_S,
        /*   10 101 000   */ XOR_S,
        /*   10 101 001   */ XOR_S,
        /*   10 101 010   */ XOR_S,
        /*   10 101 011   */ XOR_S,
        /*   10 101 100   */ XOR_S,
        /*   10 101 101   */ XOR_S,
        /*   10 101 110   */ XOR_pHL,
        /*   10 101 111   */ XOR_S,
        /*   10 110 000   */ OR_S,
        /*   10 110 001   */ OR_S,
        /*   10 110 010   */ OR_S,
        /*   10 110 011   */ OR_S,
        /*   10 110 100   */ OR_S,
        /*   10 110 101   */ OR_S,
        /*   10 110 110   */ OR_pHL,
        /*   10 110 111   */ OR_S,
        /*   10 111 000   */ CP_S,
        /*   10 111 001   */ CP_S,
        /*   10 111 010   */ CP_S,
        /*   10 111 011   */ CP_S,
        /*   10 111 100   */ CP_S,
        /*   10 111 101   */ CP_S,
        /*   10 111 110   */ CP_pHL,
        /*   10 111 111   */ CP_S,
        /*   11 000 000   */ RET_CC,
        /*   11 000 001   */ POP_QQ,
        /*   11 000 010   */ JP_CC_NN,
        /*   11 000 011   */ JP_NN,
        /*   11 000 100   */ CALL_CC_NN,
        /*   11 000 101   */ PUSH_QQ,
        /*   11 000 110   */ ADD_A_N,
        /*   11 000 111   */ RST_P,
        /*   11 001 000   */ RET_CC,
        /*   11 001 001   */ RET,
        /*   11 001 010   */ JP_CC_NN,
        /*   11 001 011   */ RunCommandWithPrefixCB,
        /*   11 001 100   */ CALL_CC_NN,
        /*   11 001 101   */ CALL_NN,
        /*   11 001 110   */ ADC_A_N,
        /*   11 001 111   */ RST_P,
        /*   11 010 000   */ RET_CC,
        /*   11 010 001   */ POP_QQ,
        /*   11 010 010   */ JP_CC_NN,
        /*   11 010 011   */ OUT_pN_A,
        /*   11 010 100   */ CALL_CC_NN,
        /*   11 010 101   */ PUSH_QQ,
        /*   11 010 110   */ SUB_N,
        /*   11 010 111   */ RST_P,
        /*   11 011 000   */ RET_CC,
        /*   11 011 001   */ EXX,
        /*   11 011 010   */ JP_CC_NN,
        /*   11 011 011   */ IN_A_pN,
        /*   11 011 100   */ CALL_CC_NN,
        /*   11 011 101   */ RunCommandWithPrefixDD,
        /*   11 011 110   */ SBC_A_N,
        /*   11 011 111   */ RST_P,
        /*   11 100 000   */ RET_CC,
        /*   11 100 001   */ POP_QQ,
        /*   11 100 010   */ JP_CC_NN,
        /*   11 100 011   */ EX_pSP_HL,
        /*   11 100 100   */ CALL_CC_NN,
        /*   11 100 101   */ PUSH_QQ,
        /*   11 100 110   */ AND_N,
        /*   11 100 111   */ RST_P,
        /*   11 101 000   */ RET_CC,
        /*   11 101 001   */ JP_pHL,
        /*   11 101 010   */ JP_CC_NN,
        /*   11 101 011   */ EX_DE_HL,
        /*   11 101 100   */ CALL_CC_NN,
        /*   11 101 101   */ RunCommandWithPrefixED,
        /*   11 101 110   */ XOR_N,
        /*   11 101 111   */ RST_P,
        /*   11 110 000   */ RET_CC,
        /*   11 110 001   */ POP_QQ,
        /*   11 110 010   */ JP_CC_NN,
        /*   11 110 011   */ DI,
        /*   11 110 100   */ CALL_CC_NN,
        /*   11 110 101   */ PUSH_QQ,
        /*   11 110 110   */ OR_N,
        /*   11 110 111   */ RST_P,
        /*   11 111 000   */ RET_CC,
        /*   11 111 001   */ LD_SP_HL,
        /*   11 111 010   */ JP_CC_NN,
        /*   11 111 011   */ EI,
        /*   11 111 100   */ CALL_CC_NN,
        /*   11 111 101   */ RunCommandWithPrefixFD,
        /*   11 111 110   */ CP_N,
        /*   11 111 111   */ RST_P
    };

    return firstLevel[PCandInc()]();
}
